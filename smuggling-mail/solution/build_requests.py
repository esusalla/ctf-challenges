...
# requests together as one. If using Burp to send the request, be sure to disable
# the "Update Content-Length" option because the first request needs to have a 
# content length of 0 for the smuggled payload to work. Additionally, a publicly 
# reachable listener must be setup prior to triggering the RCE in order to receive 
# either the flag or the reverse shell.
